...
`XMLHttpRequest` existed in browsers for a long time. `fetch` was introduced with `ES6`.

`XMLHttpRequest` was always abortable. An abortable `XHR` request looks something like this.

```javascript title="xhr-abort.js"
...
![Abort controller in the devtool](/posts/aborting-a-fetch-request/abort-controller-devtool-eval.webp)
...
```js title="abort-controller-usage.js"
...
```js title="fetch-with-abort-signal.js"
const controller = new AbortController();
const signal = controller.signal;
...
![Aborted fetch request in the devtool](/posts/aborting-a-fetch-request/fetch-devtool-cancelled.webp)
...
<CanIUse feature="abortcontroller" />
...
That's it for now. If you enjoyed reading this post, give me a follow [@ganapativs](https://x.com/ganapativs) 😬
