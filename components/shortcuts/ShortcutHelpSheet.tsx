...
/**
 * The `?` sheet itself, split out so it can be loaded on demand.
 *
 * `@base-ui/react/dialog` pulls 62 modules — the dialog, floating-ui, the
 * focus and dismiss machinery — for 25 kB gzipped, which was 14% of every
 * page's JavaScript, on every route, for a panel most readers never open.
 * ShortcutHelp is the gate; this is what it fetches.
 */
const GROUP_ORDER = ["Navigate", "Theme", "Sound", "Ink", "Page", "Help"];
        ...
        <Dialog.Popup className="khelp-popup">
          <header className="khelp-header">
            ...
            <Dialog.Description className="khelp-sub">
              Direct keys for every move. No leader, no chord.
            </Dialog.Description>
          </header>
